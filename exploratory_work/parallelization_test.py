...
class Agent:
	...
	def act(self, s, excluded_actions):
		...
		if random.random() < eps:
			...
			s = np.array([s])
			p = mybrain.predict_p(s)[0]

			# a = np.argmax(p)
			for iter in range(len(NUM_ACTIONS)):
...
class SparseAgent(base_agent.BaseAgent):
	def __init__(self):
		...
		self.learn = Agent(EPS_START, EPS_END, EPS_STEPS)
...
#TODO remove the pickling and add in h5py save model
			mybrain.model.save('a3c.h5')
